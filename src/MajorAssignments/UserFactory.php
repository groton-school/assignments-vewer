...
class UserFactory extends PDOFactory
{
    ...
    {
        $statement = $this->pdo->prepare('SELECT * FROM users WHERE user_id = :user_id AND tool_consumer_instance_guid = :tool_consumer_instance_guid LIMIT 1');
    ...
    {
        $statement = $this->pdo->prepare('UPDATE users SET ' . implode(', ', array_map(
            function ($elt) {
                return "$elt = :$elt";
            },
            array_keys($data)
        )) . ' WHERE user_id = :user_id AND tool_consumer_instance_guid = :tool_consumer_instance_guid');
        if ($statement->execute(array_merge($data, ['user_id' => $userId, 'tool_consumer_instance_guid' => $toolConsumerInstanceGUID]))) {
